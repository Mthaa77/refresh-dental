...
function SectionSkeleton({ height = 'h-40' }: { height?: string }) {
  return (
    <div className={`${height} bg-ivory/50 animate-pulse`} aria-hidden="true" />
  )
}
...
const TrustTicker = dynamic(
  ...
  { ssr: false, loading: () => <div className="h-10 bg-espresso" /> },
)
...
const ServicesGrid = dynamic(
  ...
  { ssr: true, loading: () => <SectionSkeleton height="h-[780px]" /> },
)
...
const GallerySection = dynamic(
  ...
  { ssr: false, loading: () => <SectionSkeleton height="h-[680px]" /> },
)
...
const PaymentSection = dynamic(
  ...
  { ssr: true, loading: () => <SectionSkeleton height="h-[560px]" /> },
)
  ...
  return (
    <>
      ...
      <main className="min-h-screen overflow-hidden" role="main" id="main-content">
        {/* The homepage is intentionally edited down to a focused patient journey. */}
        <Hero />
        <TrustTicker />

        {/* Meet the doctor, understand the care, then see the treatment options. */}
        <AboutSection />
        <ServicesGrid />
        <ImplantsSpotlight />
        <TechnologySection />

        {/* Make the next step feel clear, familiar and human. */}
        <TreatmentProcess />
        <GallerySection />

        {/* Independent proof and payment confidence before conversion. */}
        <Testimonials />
        <PaymentSection />
        <FAQSection />
